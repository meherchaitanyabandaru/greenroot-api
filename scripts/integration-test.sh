...
SEED_FILE="${SEED_FILE:-../greenroot-infra/db/postgresql/greenroot-seed.sql}"
...
createdb "$DB_NAME"
DATABASE_URL="$DATABASE_URL" ./scripts/migrate.sh up >/dev/null
psql -d "$DB_NAME" -v ON_ERROR_STOP=1 -f "$SEED_FILE" >/dev/null

psql -d "$DB_NAME" -v ON_ERROR_STOP=1 >/dev/null <<'SQL'
WITH upserted AS (
